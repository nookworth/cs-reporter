...
class PowerPointWriterV3(PowerPointWriter):
    ...
    def _add_charts_to_report(self, ppt_path, data):
        ...
        try:
            ...
            try:
                add_comparison_chart(
                    chart_slide, data,
                    left=0.5, top=1.5, width=4.5, height=3
                )
            except Exception as e:
                print(f"  Warning: Could not add comparison chart: {e}")
            ...
            try:
                ...
                if retail_cats:
                    add_category_pie_chart(
                        chart_slide, retail_cats, 'Retail Categories',
                        left=5.5, top=1.5, width=4, height=3
                    )
            except Exception as e:
                print(f"  Warning: Could not add category chart: {e}")
            ...
            try:
                ...
                if top_orgs:
                    add_top_orgs_chart(
                        chart_slide, top_orgs,
                        left=1.5, top=5, width=7, height=2.5
                    )
            except Exception as e:
                print(f"  Warning: Could not add top orgs chart: {e}")
            
            # Save the presentation
            prs.save(ppt_path)
            print(f"  [OK] Added charts to presentation")
