...
# This module is the bridge between raw Excel data and the PowerPoint template.
# It reads configuration mappings and extracts the exact data needed for the report.
# **Date:** 2026-02-20
# **Status:** ✅ Completed
# **Component:** src
...
from . import excel_utils  # Utility functions for Excel operations
    ...
    def _extract_field_group(self, field_group, group_name):
        ...
        for field_name, field_config in field_group.items():
            ...
            try:
                ...
                cell = field_config.get("cell")  # Column name to read from
                ...
                # SPECIAL FIELD TYPE 1: Row count fields (ticket requests)
                if field_name.endswith("_req"):
                    value = excel_utils.count_rows(excel_path, sheet)
                    data[field_name] = value
                    continue

                # SPECIAL FIELD TYPE 2: Satisfaction rating fields
                ...
                # SPECIAL FIELD TYPE 3: Resolution time fields
                ...
                # If anything goes wrong, log it but continue processing other fields
