...
def add_comparison_chart(slide, data, left, top, width, height):
...
def add_category_pie_chart(slide, table_data, title, left, top, width, height):
    ...
    for item in table_data[:5]:  # Top 5 categories
        ...
        cat_name = item.get('re_cat') or item.get('su_cat') or item.get('value', 'Unknown')
        cat_count = item.get('re_cat_count') or item.get('su_cat_count') or item.get('count', 0)
...
def add_top_orgs_chart(slide, table_data, left, top, width, height):
